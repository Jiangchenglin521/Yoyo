...
class Chatbot:
    ...
    def __init__(self):
        ...
        self.TEST_IN_NAME = 'data' + os.sep + 'test' + os.sep + 'xiaohuangji50w.txt'
        self.TEST_OUT_SUFFIX = '_predictions.txt'
        self.SENTENCES_PREFIX = ['用户: ', '邮游1号: ']
    ...
    def parseArgs(args):
        ...
        datasetArgs.add_argument('--vocabularySize', type=int, default=40, help='Limit the number of words in the vocabulary (0 for unlimited)')

        # Network options (Warning: if modifying something here, also make the change on save/loadParams() )
        nnArgs = parser.add_argument_group('Network options', 'architecture related option')
        nnArgs.add_argument('--hiddenSize', type=int, default=128, help='number of hidden units in each RNN cell')#512
        ...
        trainingArgs.add_argument('--numEpochs', type=int, default=20000, help='maximum number of epochs to run')
    ...
    def main(self, user_input, args=None):
        ...
        if self.args.test:
            ...
            #self.mainTestInteractive(self.sess)
                #remo = data.encode("utf-8")
                data = self.mainTestInteractive(self.sess,user_input)
    ...
    def predictTestset(self, sess):
        ...
        for e in range(3):
            for modelName in sorted(modelList):  # TODO: Natural sorting
                ...
                print('Testing...')
                print('Start testing (press Ctrl+C to save and exit)...')
    ...
    def mainTestInteractive(self, sess, user_input):
        ...
        if not answer:
         output='Warning: sentence too long, sorry. Maybe try a simpler sentence.'
            #elif self.args.verbose:
                        #print(self.textData.batchSeq2str(questionSeq, clean=True, reverse=True))
                        #output = self.textData.sequence2str(answer)
                        #continue  # Back to the beginning, try again
                        #print('{}{}'.format(self.SENTENCES_PREFIX[1], output))
        else:
            output=self.textData.sequence2str(answer, clean=True)
            output = id + '$'+ '1'+ '#' + output + '*the_end'
            print(output)
            print(type(output))
            output = ''.join(output.split())
            #output='TNEIb5QP4eHVN7LcHMvEKidXNvcoU6nU$1#在 想 你 啊*the_end'
            return output
